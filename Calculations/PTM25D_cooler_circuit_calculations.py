...
coolingCapacity =   [ 0.15, 0.25,   0.29,   0.31,   0.33,   0.34,   0.35,   0.358,  0.365,   0.37,   0.375,  0.378]
#dT =                [10, 20, 30, 40]

oilCooler=Cooler()
oilCooler.setName("Parker LDC 016")
...
for i in range(0, len(viscosity)):
    ...
    for flow in range(1, 150+1, 1):
        ...
        hose1 = Hose("P51P52/111/25x3200",viscosity[i], 25.0, flow, 3.2)
        elbows1 = ElbowConnectorPressureDrop(flow,1.0,25.0).calculateElbowPressureDrop()
        straights1 = StraightConnectorPressureDrop(flow,2.0,25.0).calculateStraightConnectorPressureDrop()
        pd1 = hose1.pressureDrop()+elbows1+straights1

        hose2 = Hose("Rura  22x2 CF-E355 (ocynk) PN-EN 10305 L=680",viscosity[i], 18.0, flow, 0.68)
        elbows2 = ElbowConnectorPressureDrop(flow,4.0,18.0).calculateElbowPressureDrop()
        straights2 = StraightConnectorPressureDrop(flow,2.0,18.0).calculateStraightConnectorPressureDrop()
        pd2 = hose2.pressureDrop()+elbows2+straights2

        hose3 = Hose("P51P52/111/25x1800",viscosity[i], 25.0, flow, 1.8)
        elbows3 = ElbowConnectorPressureDrop(flow,5.0,25.0).calculateElbowPressureDrop()
        straights3 = StraightConnectorPressureDrop(flow,4.0,25.0).calculateStraightConnectorPressureDrop()
        pd3 = hose3.pressureDrop()+elbows3+straights3

        hose4 = Hose("Rura  22x2 CF-E355 (ocynk) PN-EN 10305 L=680",viscosity[i], 18.0, flow, 0.68)
        elbows4 = ElbowConnectorPressureDrop(flow,4.0,18.0).calculateElbowPressureDrop()
        straights4 = StraightConnectorPressureDrop(flow,1.0,18.0).calculateStraightConnectorPressureDrop()
        pd4 = hose4.pressureDrop()+elbows4+straights4

        hose5 = Hose("P51P52/111/25x1300", viscosity[i], 25.0, flow, 1.3)
        elbows5 = ElbowConnectorPressureDrop(flow, 1.0, 25.0).calculateElbowPressureDrop()
        straights5 = StraightConnectorPressureDrop(flow, 1.0, 25.0).calculateStraightConnectorPressureDrop()
        pd5 = hose5.pressureDrop() + elbows5 + straights5

        totalPressureDrop = pd1+pd2+pd3+pd4+pd5
